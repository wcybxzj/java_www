...
public class PayController {
    ...
    //127.0.0.1:8080/api5?tag=order_pay
    //127.0.0.1:8080/api5?tag=order_finish
    //127.0.0.1:8080/api5?tag=order_no
    //tag例子
    @RequestMapping("/api5")
    public Object callback5(String tag) throws Exception {
        Message message = new Message(JmsConfig.TOPIC, tag ,"" ,tag.getBytes());
        SendResult sendResult  = payProducer.getProducer().send(message);
        System.out.printf("生产者:发送结果=%s, msg=%s ", sendResult.getSendStatus(), sendResult.toString());
        return new HashMap<>();
    }


    //sql92
    //http://127.0.0.1:8080/api6?tag=order_finish&amount=5
    //http://127.0.0.1:8080/api6?tag=order_finish&amount=6
    @RequestMapping("/api6")
    public Object callback6(String tag, String amount) throws Exception {
        Message message = new Message(JmsConfig.TOPIC, tag ,"" ,tag.getBytes());
